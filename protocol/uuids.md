...
Приложение **обязано** использовать Service UUID `7C9A0001-…` как авторитетный
признак совместимости (ТЗ §21). На Android допустим нефильтрованный platform scan с
локальным UUID-фильтром. Стандартное имя `BikeComp-*` может временно добавить
кандидата, если Android не декодировал UUID из рекламы; такой кандидат обязан пройти
полный GATT discovery до перехода в рабочее состояние.
