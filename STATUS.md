...
Э2 — базовая прошивка. Нормативный контракт дисплея обновляется под постоянную
верхнюю зону OLED (скорость и батарея) и пять страниц нижней карусели.
...
Реализовать `PageCarousel`, общий форматтер C++ и синхронные golden-кадры симулятора.
